 ...
 */
public class HostVulnsFilterConfidentiality extends Fragment implements View.OnClickListener {
    ...
    public HostVulnsFilterConfidentiality() {
    ...
    public void onViewCreated(View view, Bundle savedInstancesState) {
        ...
        title.setText("Confidentiality Impact for host: " + host);

        PieChartCreator pieChartCreator = new PieChartCreator();

        Map<String, Float> confidentialityScore = resultController.getConfidentialityScore(host);
        confidentialityScore.remove(VulnerabilityResult.NONE);

        PieChart confidentialityChart = pieChartCreator.createChart(context, 1000, 1000);
        pieChartCreator.setChartConfig(ChartDescription.CONFIDENTIALITY, true, LegendHeadings.CONFIDENIALITY, confidentialityChart, confidentialityScore, PieChartCreator.LOW_TO_CRITICAL);
        mainLayout.addView(confidentialityChart);


        TableCreator tableCreator = new TableCreator();
        tableCreator.appendTableHeader(context, mainLayout, TableHeadings.VULN_NAME, TableHeadings.CONFIDENTIALITY_IMPACT, TableHeadings.RISK_SCORE);
        ...
        for (VulnerabilityResult result : vulnerabilityResults) {
            String score = result.getConfidentialityScore();
            switch (score) {
        ...
        for (VulnerabilityResult result : resultsSorted) {
            ...
            StyledText styledConfidentiality = result.getConfidentialityScoreStyledText();
            ...
            TableRowData tableRowDataImpact = new TableRowData(styledConfidentiality.getText(), styledConfidentiality.getStyle(), Gravity.CENTER);
    ...
    public static HostVulnsFilterConfidentiality newInstance(ScanResults scanResults, String host) {
        HostVulnsFilterConfidentiality fragment = new HostVulnsFilterConfidentiality();
