...
public class UserSecurityAwareness extends Fragment implements View.OnClickListener {
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        layoutVideos = view.findViewById(R.id.nav_videos);
        ...
        SpannableString customQuiz = new SpannableString(Html.fromHtml(" <u> start custom quiz </u>"));
        ClickableSpan clickableText = new ClickableSpan() {
            @Override
            public void onClick(View widget) {
                Intent awarenessActivity = new Intent(context, UserAwareness.class);
                awarenessActivity.putExtra(AppStorage.START_CUSTOM_QUIZ, true);
                startActivity(awarenessActivity);
            }
        };
        customQuiz.setSpan(clickableText, 0, customQuiz.toString().length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        customQuiz.setSpan(new ForegroundColorSpan(Color.BLUE), 0, customQuiz.toString().length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        spannableStringBuilder.append(getString(R.string.results_recommended_quiz));
        spannableStringBuilder.append(customQuiz);
        TextView textViewCustomQuiz = new TextView(context);
        textViewCustomQuiz.append(spannableStringBuilder);
        textViewCustomQuiz.setMovementMethod(LinkMovementMethod.getInstance());
        quizLayout.addView(textViewCustomQuiz);
    }
